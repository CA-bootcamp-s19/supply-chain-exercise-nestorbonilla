...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier verifyCaller (address _address) {
    require (msg.sender == _address, 'Sender is not the same');
    _;
  }

  modifier paidEnough(uint _price) {
    require(msg.value >= _price, 'Price is higher than value provided');
    _;
  }
  ...
  modifier forSale(uint sku) {
    require(items[sku].seller != address(0) && items[sku].state == State.ForSale, 'Item is not available for sale');
    _;
  }
  ...
  modifier received(uint sku) {
    require(items[sku].state == State.Received, 'Item hast not been received');
    _;
  }
